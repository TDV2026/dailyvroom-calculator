      ...
      if (fxRes.ok) {
        ...
        rates = Object.fromEntries(
          Object.keys(FALLBACK_RATES).map((c) => [
            c,
            Math.round((1 / raw[c]!) * 10000) / 10000,
          ]),
        );
...
${
  tariffs
    ? `Tariff rates as of ${tariffsDate}:
...
Use these exact rates. Do not use any other rates or figures.`;
          ...
          if (row_id) {
            ...
            if (Object.keys(updateData).length > 0) {
              const sbRes = await fetch(
                `${process.env.SUPABASE_URL}/rest/v1/calculations?id=eq.${row_id}`,
                {
                  method: "PATCH",
                  headers: {
              ...
              logger.info({ status: sbRes.status }, "Supabase PATCH");
            }
          }
        } catch (logErr) {
          logger.error({ err: logErr }, "Supabase update error");
        }
      }
    ...
    const enrichedMessages = (messages || []).map((msg: any) =>
      msg.role === "user" ? { ...msg, content: msg.content + rateContext } : msg,
    );
